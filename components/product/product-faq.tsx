...
interface ProductFAQProps {
  productTitle: string;
  brandName: string;
  /** JSON string from custom.faqs metafield — if present, used instead of generated FAQs */
  faqsJson?: string | null;
}

function parseFaqsJson(json: string | null | undefined): FAQ[] | null {
  if (!json) return null;
  try {
...
export function ProductFAQ({ productTitle, brandName, faqsJson }: ProductFAQProps) {
  const [openIndex, setOpenIndex] = useState<number | null>(0);

  const realFaqs = parseFaqsJson(faqsJson);
  const faqs = realFaqs ?? generateFAQs(productTitle, brandName);
  const isRealData = realFaqs !== null;
        ...
        <div>
          ...
          {isRealData && (
            <p className="text-xs text-muted-foreground mt-0.5">
              Risposte generate dall&apos;analisi delle recensioni e domande reali dei clienti
            </p>
          )}
        </div>
      </div>
