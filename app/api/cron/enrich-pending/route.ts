...
async function getUnenrichedProducts(limit: number = 50): Promise<any[]> {
  const shopDomain = optionalEnv.SHOPIFY_SHOP_DOMAIN;
  const accessToken = env.SHOPIFY_ADMIN_ACCESS_TOKEN;

  if (!shopDomain) {
    throw new Error('SHOPIFY_SHOP_DOMAIN not configured');
  }
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const baseUrl = optionalEnv.NEXT_PUBLIC_BASE_URL || 'https://autonord-shop.vercel.app';
