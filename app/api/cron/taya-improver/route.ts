...
export async function GET(request: NextRequest) {
  ...
  if (authHeader !== `Bearer ${env.CRON_SECRET}` && querySecret !== env.CRON_SECRET) {
...
Rispondi SOLO con JSON valido nel formato:
{
  ...
  "summary": "Riepilogo generale dello stato di compliance TAYA"
    ...
    if (optionalEnv.RESEND_API_KEY && optionalEnv.NOTIFICATION_EMAIL) {
      try {
...
async function sendReportEmail(plan: ImprovementPlan): Promise<void> {
  ...
  const toEmail = optionalEnv.NOTIFICATION_EMAIL;
