...
async function runEnrichmentPipeline(payload: WorkerPayload, startTime: number): Promise<Response> {
    ...
    const [qaOutput, imageResult] = await Promise.all([
      ...
      (async () => {
        // Guard: skip only when BOTH the flat string AND structured sources are absent/tiny.
        // Previously only sourceData.length was checked, so QA was skipped when RAG succeeded
        // with structured sources but produced a short flat string (e.g. after granularity trim).
        const hasEnoughData =
    ...
    // ===========================================
    // STEP 5.5: Verifier (CLaRa) — Fact coverage + consistency check.
    // Non-blocking: only regenerates on content with critical inconsistencies.
    // Resolves audit items R15, R19, R12 (post-generation fact-check).
    // ===========================================
    if (qaResult) {
      try {
        console.log('[Worker V5] Step 5.5: Running Verifier (fact coverage + consistency)...');
        const verifierFacts = qaResult.simpleQA.rawFacts
        ...
        if (verifierFacts.length >= 3) {
          ...
          const verifierSourceData = {
            title: payload.title,
            brand: payload.vendor,
            sku: payload.sku || '',
            rawSpecs: verifierSpecs,
            researchData: adaptation.qaInput?.sourceData ?? '',
            verifiedFacts: verifierFacts,
          };
          const verifierResult = await verifyAndRegenerateLoop(
            verifierContent,
            verifierSourceData,
            2  // max 2 attempts: verify once + regenerate once if needed
          );
          if (verifierResult.improved) {
            console.log(`[Worker V5] ✅ Step 5.5 Verifier improved content (attempts=${verifierResult.attempts}, score=${verifierResult.verification.overallScore})`);
            // Update validated content with verifier improvements (mutable properties)
            validationResult.content.description = verifierResult.finalContent.description;
            validationResult.content.pros = verifierResult.finalContent.pros;
            validationResult.content.cons = verifierResult.finalContent.cons;
            validationResult.content.faqs = verifierResult.finalContent.faqs;
          } else {
            console.log(`[Worker V5] Step 5.5 Verifier: content OK (score=${verifierResult.verification.overallScore})`);
          }
        } else {
          console.log('[Worker V5] Step 5.5: Verifier skipped (< 3 verified facts available)');
        }
      } catch (verifierErr) {
        console.warn('[Worker V5] Step 5.5: Verifier failed (non-blocking):', verifierErr instanceof Error ? verifierErr.message : String(verifierErr));
      }
    }
