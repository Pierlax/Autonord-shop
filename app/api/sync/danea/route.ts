...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (contentType.includes('multipart/form-data')) {
      ...
      // Raw CSV content — read as buffer first so we can check size BEFORE decoding
      ...
      const csvContent = decodeFileBuffer(rawBuffer);
      products = parseDaneaCSV(csvContent);

    } else if (contentType.includes('application/json')) {
      // JSON with CSV content (legacy) — check buffer size before deserialising
      const rawBuffer = await request.arrayBuffer();
      if (rawBuffer.byteLength > MAX_FILE_SIZE_BYTES) {
        return NextResponse.json(
          { error: 'Payload Too Large', message: 'File must be under 50 MB' },
      ...
      const body = JSON.parse(new TextDecoder().decode(rawBuffer)) as { csv?: string; content?: string };
      const csvContent = body.csv || body.content || '';
      products = parseDaneaCSV(csvContent);

    } else {
      // Fallback: sniff by magic bytes — enforce size limit before processing
      const buffer = await request.arrayBuffer();
      if (buffer.byteLength > MAX_FILE_SIZE_BYTES) {
        return NextResponse.json(
          { error: 'Payload Too Large', message: 'File must be under 50 MB' },
