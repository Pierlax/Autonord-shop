...
import { sanitize, sanitizeInline } from '@/lib/sanitize-html';
...
export default async function BlogPostPage({ params }: BlogPostPageProps) {
  ...
  const renderMarkdownContent = (content: string) => {
    return content
      ...
      .map((line, index) => {
        ...
        // Bold text — sanitized before injection
        const processedLine = sanitizeInline(
          line.replace(/\*\*(.*?)\*\*/g, '<strong class="text-white font-semibold">$1</strong>')
        );
        ...
        if (line.startsWith('- ')) {
          return (
            <li key={index} className="text-zinc-300 ml-4 mb-2"
                dangerouslySetInnerHTML={{ __html: sanitizeInline(processedLine.slice(2)) }} />
          );
        }
        ...
        if (numberedMatch) {
          return (
            <li key={index} className="text-zinc-300 ml-4 mb-2 list-decimal"
                dangerouslySetInnerHTML={{ __html: sanitizeInline(processedLine.slice(numberedMatch[0].length)) }} />
          );
        }
  ...
  const BASE_URL = 'https://autonordservice.com';
        ...
        <div className="grid lg:grid-cols-4 gap-12">
          ...
          <article className="lg:col-span-3">
            ...
            <div className="prose prose-invert prose-lg max-w-none">
              {isHtmlContent ? (
                // Render HTML content from Shopify — sanitized to prevent XSS
                <div
