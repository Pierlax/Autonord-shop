 ...
 *   • Redis available → uses the ZSET expiry index via `evictExpiredByIndex()`.
 *     O(log N + K) where K = number of expired entries. No full scan.
 *   • Redis absent (local dev) → falls back to loading all entries and filtering
 *     by `expiresAt`. O(N), acceptable for small local stores.
 */
export async function cleanupExpiredMemories(): Promise<{
  deleted: number;
  deletedIds: string[];
}> {
  // Fast path: use the ZSET index (Redis only)
  const redisResult = await evictExpiredByIndex();
  if (redisResult.deleted > 0 || process.env.UPSTASH_REDIS_REST_URL) {
    // Redis is configured — index-based eviction is authoritative
    return redisResult;
  }

  // Slow path: file-based fallback (local dev, no Redis)
 ...
 */
export async function cleanupLowQualityMemories(options: {
  ...
  for (const memory of memories) {
    ...
    const quality = await calculateMemoryQuality(memory);
 ...
 */
export async function executeConsolidation(candidate: ConsolidationCandidate): Promise<{
  success: boolean;
  keptId: string;
  deletedIds: string[];
  error?: string;
  ...
  try {
    ...
    for (const memory of toDelete) {
      if (await deleteMemory(memory.id)) {
        deletedIds.push(memory.id);
      }
    }
 ...
 */
export async function runFullMaintenance(options: {
  ...
  // 1. Cleanup expired
  if (!dryRun) {
    const expiredResult = await cleanupExpiredMemories();
  ...
  const lowQualityResult = await cleanupLowQualityMemories({ dryRun });
  ...
  if (consolidationCandidates.length > 0) {
    if (autoConsolidate && !dryRun) {
      for (const candidate of consolidationCandidates) {
        const result = await executeConsolidation(candidate);
        report.actions.push({
          type: 'consolidate',
 ...
 */
export async function getMemoryHealthReport(): Promise<{
  ...
  if (memories.length > 0 && lowQualityCount > memories.length * 0.3) {
