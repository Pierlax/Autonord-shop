...
function buildBriefPrompt(topic: TopicAnalysis, discovery: BlogDiscoveryResult): string {
  ...
  // W-BR-6: Visual clues for image context
  const visualCluesBlock = discovery.visualClues && discovery.visualClues.length > 0
    ? `\n## VISUAL CONTEXT (suggerimenti immagini)\n${discovery.visualClues.join(', ')}`
    : '';
...
${graphContext}${conflictBlock}${visualCluesBlock}
