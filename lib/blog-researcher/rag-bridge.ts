...
export interface BlogDiscoveryResult {
  ...
  debugLog: string[];
  /**
   * W-BR-6: Visual clues extracted from the corpus for image context.
   * Contains color descriptors, form-factor hints, action-shot keywords,
   * and product codes found in forum threads and editorial articles.
   * Passed downstream to drafting-v2 / image search queries.
   */
  visualClues: string[];
}
...
// ---------------------------------------------------------------------------
// W-BR-6: Visual clue extractor
// ---------------------------------------------------------------------------

/**
 * W-BR-6: Extract visual clues from the blog corpus for image suggestions.
 *
 * Unlike the product pipeline (image-agent-v4 with full vision validation),
 * the blog pipeline had no visual context at all. This function scans corpus
 * items for colour descriptors, form-factor hints, action-shot keywords,
 * and product codes, returning up to 20 short strings that downstream
 * image search / article prompts can use as context.
 */
function extractVisualClues(
  sources: BlogDiscoveredSource[],
  corpus: CorpusCollection
): string[] {
  const clues = new Set<string>();

  const COLOR_RE    = /\b(rosso|blu|giallo|nero|grigio|arancione|verde|red|blue|yellow|black|grey|gray|orange|green)\b/gi;
  const FORM_RE     = /\b(compatto|compatta|ergonomico|ergonomica|slim|leggero|leggera|pistola|angolare|right[- ]angle|sds)\b/gi;
  const ACTION_RE   = /\b(in uso|in cantiere|sul lavoro|in action|in use|on site|being used|cutting|drilling|grinding|smontaggio|montaggio)\b/gi;
  const CODE_RE     = /\b([A-Z]{1,4}[\s\-]?\d{3,7}[A-Z0-9\-]{0,6})\b/g;
  ...
  for (const item of corpus.items) {
    ...
    while ((m = COLOR_RE.exec(text)) !== null) clues.add(`colore: ${m[1].toLowerCase()}`);

    FORM_RE.lastIndex = 0;
    while ((m = FORM_RE.exec(text)) !== null) clues.add(`forma: ${m[1].toLowerCase()}`);

    ACTION_RE.lastIndex = 0;
    while ((m = ACTION_RE.exec(text)) !== null) clues.add(`azione: ${m[1].toLowerCase()}`);
  }

  // Extract product codes from source snippets for image-search context
  for (const src of sources.slice(0, 15)) {
    CODE_RE.lastIndex = 0;
    const matches = src.snippet.match(CODE_RE);
    if (matches) {
      for (const code of matches.slice(0, 3)) clues.add(`codice: ${code}`);
    }
  }

  return Array.from(clues).slice(0, 20);
}
...
export async function discoverBlogSources(
  ...
  // R30: Search call cap — prevent quota explosion
  ...
  // R30: Count approximate search calls used so far
  searchCallsUsed = intents.length * 3; // ~3 queries per intent
  debug.push(`[R30] Search calls after L2: ~${searchCallsUsed}/${MAX_SEARCH_CALLS}`);
  ...
  if (evaluation.needsSecondPass && evaluation.gaps.length > 0) {
    ...
    for (const query of gapQueries) {
      // R30: Skip if search call budget exhausted
      if (searchCallsUsed >= MAX_SEARCH_CALLS) {
        debug.push(`[R30] Search cap reached (${searchCallsUsed}/${MAX_SEARCH_CALLS}) — skipping remaining gap queries`);
        log.warn(`[RagBridge] R30 Search call cap reached at ${searchCallsUsed} calls — stopping second pass early`);
        break;
      }
  ...
  // W-BR-6: Extract visual clues from corpus for downstream image context
  const visualClues = extractVisualClues(allSources, corpusCollection);
  if (visualClues.length > 0) {
    debug.push(`[W-BR-6] visual clues: ${visualClues.length} extracted`);
    log.info(`[RagBridge] W-BR-6: ${visualClues.length} visual clues extracted`);
  }
