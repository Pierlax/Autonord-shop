...
/**
 * Domains blocked from image discovery.
 *
 * Criteria for inclusion:
 * a) Marketplace with no brand-authorised product images (Amazon, eBay, Temu, etc.)
 * b) Social media — images are user-generated, resized, or watermarked
 * c) Domains known to serve generic placeholder images for unknown products
 *
 * W-IA-6: List expanded to cover new problematic marketplaces.
 * This list is intentionally static — these are structurally bad sources,
 * not transient failures. Transient failures are handled by isImageAccessible().
 */
const BLOCKED_DOMAINS = [
  // Major marketplaces (no brand-controlled images)
  'amazon.', 'ebay.', 'aliexpress.',
  // W-IA-6: Additional marketplaces known for low-quality/unverifiable images
  'temu.com', 'shein.com',
  'joom.com', 'banggood.com',
  'dhgate.com', 'lightinthebox.com',
  'wish.com',
  // Social media — resized, watermarked, or user-generated
...
// =============================================================================
// R25: CROSS-CODE RESULT CACHE
// =============================================================================

/**
 * R25: In-memory cache for findAlternativeCodes() results.
 * Key: "{brand.toLowerCase()}|{mpnPrefix}" — same brand usually has similar code patterns.
 * This avoids redundant Gemini calls for brands that have no international variants.
 */
const _altCodeCache = new Map<string, string[]>();

// =============================================================================
// R24: STEP 6 CIRCUIT BREAKER
// =============================================================================
...
export interface ImageAgentV4Result {
  ...
  /** D24 fix: Image quality score 0-1 based on URL heuristics */
  qualityScore?: number;
  error?: string;
}
...
export async function findProductImage(
  ...
  const result = await cachedGenericDynamic<ImageAgentV4Result>(
    cacheKey,
    ...
    (r) => r.success ? IMAGE_SUCCESS_TTL_MS : IMAGE_FAILURE_TTL_MS,
  );

  // D24 fix: Attach quality score to successful results
  if (result.success && result.imageUrl && result.qualityScore === undefined) {
    result.qualityScore = scoreImageQuality(result.imageUrl, result.source);
  }
...
async function findProductImageUncached(
  ...
  if (realRagPageUrls.length > 0) {
    console.log(`[ImageAgent V4] 🔍 STEP 1.5: Trying ${realRagPageUrls.length} RAG-discovered pages (parallel)...`);

    // D21 fix: Fetch og:image from all RAG pages in parallel instead of sequentially.
    // Previously each page was fetched one-by-one (10s timeout × N pages = up to 80s).
    // Now all pages are fetched concurrently with Promise.allSettled — total wall-clock
    // time is max(individual timeouts) ≈ 10s instead of sum.
    const ogResults = await Promise.allSettled(
      realRagPageUrls.map(async (pageUrl) => {
        const ogResult = await fetchOgImageFromPage(pageUrl, brand, identifiers.allCodes, title, mpnCompact);
        return ogResult ? { pageUrl, ogResult } : null;
      })
    );

    // Process results in order (preserve RAG priority ranking)
    for (const settled of ogResults) {
      if (settled.status !== 'fulfilled' || !settled.value) continue;
      const { pageUrl, ogResult } = settled.value;
      ...
      // W-IA-3: Skip vision validation when source is an official brand domain AND
      // page title gave a strong text_match (model code confirmed in title → ~$0.002 saved).
      // text_match + official domain = very high confidence, vision is redundant.
      const skipVision = isOfficial && !escalated;
      const visionOk = skipVision || await validateImageWithVision(ogResult.imageUrl, title, brand, visualClues);
      if (visionOk) {
        console.log(`[ImageAgent V4] ✅ RAG page og:image: ${ogResult.imageUrl.substring(0, 100)} (${domain}, match=${ogResult.matchReason}, visionSkipped=${skipVision})`);
        return {
          success: true,
          ...
          provenance: {
            ...
            matchReason: skipVision ? 'high_confidence_skip' : (escalated ? 'vision_escalated' : 'text_match'),
            escalatedToVision: !skipVision && escalated,
          },
        };
      }
    }
  ...
  if (officialResult.found && officialResult.imageUrl && await isImageAccessible(officialResult.imageUrl)) {
    // W-IA-3: Skip vision if the product code appears in the image URL path — that is a
    // direct code-in-URL identity signal on a trusted official domain, equivalent to
    // GCS confidence='high'. Vision would be redundant and costs ~$0.002.
    const officialImgUrlLower = officialResult.imageUrl.toLowerCase();
    const codeInUrl = identifiers.allCodes.some(c =>
      c.length >= 6 && officialImgUrlLower.includes(c.toLowerCase().replace(/[\s\-]/g, ''))
    );
    const visionOk = codeInUrl
      || await validateImageWithVision(officialResult.imageUrl, title, brand, visualClues);
    if (visionOk) {
      console.log(`[ImageAgent V4] ✅ Official site image found: ${officialResult.domain} (visionSkipped=${codeInUrl})`);
      return {
        success: true,
        ...
        provenance: {
          ...
          sourcePageUrl: null,
          sourcePageTitle: null,
          matchReason: codeInUrl ? 'code_in_url' : 'vision_escalated',
          escalatedToVision: !codeInUrl,
        },
      };
    }
...
async function searchGCSImageDirect(
  ...
  brand: string,
  codes: string[],
  category: string | null // W-IA-5: was _category (unused) — now used for codeless product fallback
  ...
  // W-IA-5: For codeless products (no alphanumeric model code — only EAN or nothing),
  // augment the domain list with Italian specialist retailers that carry more accessories
  // and consumables than UK/US tool shops.
  const hasMpnLike = codes.some(c => c.length >= 5 && /[A-Z]/i.test(c) && /\d/.test(c));
  if (!hasMpnLike && category) {
    const accessoryDomains = ['rotopino.it', 'fixami.it', 'totalutensili.it', 'utensileriaonline.it'];
    for (const d of accessoryDomains) {
      if (!imageDomains.includes(d)) imageDomains.push(d);
    }
    imageDomains = imageDomains.slice(0, 6); // keep list manageable
  }
  ...
  const titleWords = title
    ...
    .split(/[\s\-\/,]+/)
    .filter(w => w.length >= 3 && !/^(per|con|the|and|kit|set|pro|new|da|di|il|la|le|un|una|e|a|in|su)$/i.test(w))
    .slice(0, 4);
  queries.push(`${brand} ${titleWords.join(' ')}`);

  // W-IA-5: For codeless products (accessories, consumables, spare parts), prepend a
  // quoted-title query that uses descriptive keywords instead of model codes.
  // This runs BEFORE the generic title fallback so the category domain hints are in scope.
  if (!hasMpnLike && titleWords.length >= 2) {
    const quotedTitleQuery = `"${brand}" "${titleWords.slice(0, 3).join(' ')}"`;
    queries.unshift(quotedTitleQuery);
    console.log(`[ImageAgent V4] W-IA-5: Codeless product — using title query: ${quotedTitleQuery} on ${imageDomains.slice(0, 4).join(', ')}`);
  }
...
/**
 * W-IA-1: Validates the format of an AI-generated alternative product code.
 *
 * Gemini can invent plausible-looking codes that don't actually exist
 * (e.g. "MILW-X1234", "M18-FPD9-0X" for a non-existent variant).
 * This filter rejects codes that fail brand-specific format rules,
 * reducing hallucinated codes before they pollute the search pipeline.
 *
 * For Milwaukee the formats are well-defined; other brands use a generic
 * alphanumeric-plus-length check that still catches obvious hallucinations
 * (pure words, excessively short/long, non-ASCII).
 */
function validateAiGeneratedCode(code: string, brand: string): boolean {
  if (!code || typeof code !== 'string') return false;
  // Length guard: product codes are 4–25 chars
  const trimmed = code.trim();
  if (trimmed.length < 4 || trimmed.length > 25) return false;
  // Must contain at least one letter AND one digit (all real product codes do)
  if (!/[A-Z]/i.test(trimmed) || !/\d/.test(trimmed)) return false;
  // Only alphanumeric, dashes, and spaces (no slashes, punctuation, etc.)
  if (!/^[A-Z0-9\s\-]+$/i.test(trimmed)) return false;

  const brandLower = brand.toLowerCase();

  // Milwaukee: three known formats — EU article number, US dash format, model code
  if (brandLower.includes('milwaukee')) {
    const isEuArticle = /^\d{10}$/.test(trimmed);               // 4932471068
    const isUsFormat  = /^4[89]-\d{2}-\d{4}$/.test(trimmed);   // 48-32-4006
    const isModelCode = /^(?:M1[28]|M28)\s*[A-Z]{2,}/i.test(trimmed); // M18FID2-0X
    if (!isEuArticle && !isUsFormat && !isModelCode) return false;
  }
...
async function findAlternativeCodes(
  ...
  // R25: Check cache first — same brand+prefix likely has same code patterns
  const cacheKey = `${brand.toLowerCase().slice(0, 20)}|${mpn.slice(0, 6)}`;
  const cached = _altCodeCache.get(cacheKey);
  if (cached !== undefined) {
    if (cached.length > 0) {
      console.log(`[ImageAgent V4] Cross-code cache hit: ${cacheKey} → ${cached.join(', ')}`);
    }
    return cached.filter(c => c !== mpn);
  }
...
If no alternatives found, return: []`,
    ...
    if (match) {
      ...
      const filtered = (codes as unknown[]).filter((c): c is string => typeof c === 'string' && c !== mpn);
      // W-IA-1: Validate format of AI-generated codes to filter hallucinations.
      // Rejects codes that don't match known brand format patterns.
      const validated = filtered.filter(c => validateAiGeneratedCode(c, brand));
      if (validated.length < filtered.length) {
        const dropped = filtered.filter(c => !validateAiGeneratedCode(c, brand));
        console.warn(
          `[ImageAgent V4] W-IA-1: Dropped ${dropped.length} invalid-format AI code(s) for ${brand}: [${dropped.join(', ')}]`
        );
      }
      _altCodeCache.set(cacheKey, validated); // R25: cache result
      return validated;
    }
  } catch (e) {
    console.log(`[ImageAgent V4] Cross-code search failed: ${e}`);
    _altCodeCache.set(cacheKey, []); // R25: cache empty result to avoid retry storms
  }
...
async function searchDirectUrls(
  ...
  // D22 fix: Track success/failure per retailer domain to detect broken URL patterns.
  // When a domain fails consistently (5+ consecutive failures), log a warning
  // so broken patterns are visible in monitoring.
  const domainFailures: Record<string, number> = {};

  // Try each URL — stop at the first valid og:image
  for (const candidate of candidateUrls) {
    // D22: Skip domains that have failed too many times in this run
    if ((domainFailures[candidate.domain] ?? 0) >= 2) {
      console.log(`[ImageAgent V4] D22: Skipping ${candidate.domain} — already failed ${domainFailures[candidate.domain]}× in this run`);
      continue;
    }
    ...
    try {
      ...
      // D22: Track failure
      domainFailures[candidate.domain] = (domainFailures[candidate.domain] ?? 0) + 1;
    } catch (err) {
      domainFailures[candidate.domain] = (domainFailures[candidate.domain] ?? 0) + 1;
  ...
  // D22 fix: Log domains with high failure rates for monitoring/alerting
  for (const [domain, failures] of Object.entries(domainFailures)) {
    if (failures >= 2) {
      console.warn(
        `[ImageAgent V4] D22 URL PATTERN ALERT: ${domain} failed ${failures}× — ` +
        `URL pattern may be broken, review searchDirectUrls() for this domain`
      );
    }
  }
 ...
 *
 * W-IA-4: Extended timeout (5s → 8s) + HEAD→GET fallback.
 * Some CDNs with redirect chains or Cloudflare protection reject HEAD requests
 * (405 Method Not Allowed) but serve images fine on GET. The GET retry uses
 * Range: bytes=0-0 to avoid downloading the full image — only the Content-Type
 * header is needed.
 */
async function isImageAccessible(url: string): Promise<boolean> {
  ...
  // W-IA-4: Helper to probe a URL and return { status, contentType } or null on error
  const probe = async (method: 'HEAD' | 'GET'): Promise<{ status: number; ct: string } | null> => {
    try {
      const res = await fetch(url, {
        method,
        signal: AbortSignal.timeout(8000), // W-IA-4: extended from 5s to 8s
        headers: {
          'User-Agent': 'Mozilla/5.0 (compatible; Autonord-Bot/1.0)',
          // For GET: request only the first byte — we just need Content-Type, not the body
  ...
  // W-IA-4: Retry with GET if HEAD timed out or returned 405 (Method Not Allowed)
  if (!result || result.status === 405) {
    console.log(
      `[ImageAgent V4] W-IA-4: HEAD ${result ? '405' : 'timeout'} for ${url.substring(0, 60)} — retrying with GET`
    );
    result = await probe('GET');
  }

  if (!result) {
    console.warn('[ImageAgent V4] HEAD+GET probe both failed — treating URL as invalid');
    return false;
  }

  // Definitive 404/410 → skip; other failures → fail-open (let Shopify validate on upload)
  if (result.status === 404 || result.status === 410) return false;
  if (result.ct && !result.ct.startsWith('image/') && !result.ct.includes('octet-stream') && result.ct !== '') return false;
  return true;
}
...
/**
 * D23 fix: In-process cache for Gemini Vision validation results.
 *
 * The same image URL can be validated 3-5 times during a single run
 * (RAG pages, direct URLs, GCS results all feeding the same candidate).
 * Each call costs ~$0.003 and 2-5s. Caching by imageUrl eliminates
 * redundant calls and makes results deterministic within a run.
 */
 ...
 *
 * D23 fix: Results are cached in-process by imageUrl to avoid
 * re-validating the same image (saves ~$0.003 + 2-5s per duplicate call).
 */
async function validateImageWithVision(
  ...
  // D23 fix: Return cached result if available
  const cached = visionCache.get(imageUrl);
  if (cached !== undefined) {
    console.log(`[ImageAgent V4] D23: Vision cache hit for ${imageUrl.substring(0, 60)} → ${cached ? 'MATCH' : 'NO_MATCH'}`);
    return cached;
  }
...
Reply with exactly one word: MATCH or NO_MATCH.`
  ...
  try {
    ...
    // D23 fix: Cache result for this imageUrl
    visionCache.set(imageUrl, valid);
    return valid;
  } catch (err) {
    console.log(`[ImageAgent V4] Vision validation error (assuming valid): ${err}`);
    // D23: Don't cache errors — they may be transient
...
/**
 * D24 fix: Estimate image quality from URL heuristics (0-1 score).
 *
 * Since we can't inspect image pixels without downloading, we use URL signals:
 * - File extension preference: jpg/png > webp > gif
 * - URL size hints: large/original/full > thumbnail/small
 * - CDN quality params: q=100 > q=50
 * - Source domain trust: official brand sites > retailers > generic
 * - White background indicators: /product-image/, /catalog/
 * - Negative signals: thumbnail, _small, _thumb, watermark
 *
 * W-IA-2: Added domain-specific CDN bonuses (CDNs known to consistently serve
 * 800×800 white-background product shots) and lifestyle/hero shot penalty.
 */

/**
 * W-IA-2: CDN domains known to consistently serve high-quality product images
 * (typically 800×800 white background, professionally lit).
 */
const HIGH_QUALITY_CDN_DOMAINS = [
  'toolstop.co.uk', 'ffx.co.uk',       // UK — reliable 800×800 catalog shots
  'powertoolworld.co.uk',
  'screwfix.com', 'toolstation.com',
];

/**
 * W-IA-2: URL path patterns that indicate professional product photography
 * (Magento media catalog, Shopify product CDN, etc.).
 */
const PRODUCT_PHOTO_PATH_RE = /\/(products?|catalog\/product|media\/catalog|images?\/product[s]?|product[_-]images?|store\/media)\//i;

function scoreImageQuality(imageUrl: string, sourceDomain?: string | null): number {
  const lower = imageUrl.toLowerCase();
  let score = 0.5; // Baseline

  // Positive: large/original image indicators
  if (/\b(original|full[_-]?size|large|hi[_-]?res|max|1200|1500|2000)\b/.test(lower)) score += 0.15;
  if (/\b(product[_-]?image|catalog|main[_-]?image)\b/.test(lower)) score += 0.10;

  // W-IA-2: Bonus for product photography URL path patterns
  if (PRODUCT_PHOTO_PATH_RE.test(imageUrl)) score += 0.10;

  // Positive: high quality file format
  if (/\.(jpg|jpeg|png)(\?|$)/.test(lower)) score += 0.05;

  // Positive: CDN quality parameters suggesting high quality
  if (/[?&](q|quality)=(9\d|100)\b/.test(lower)) score += 0.05;
  if (/[?&](w|width)=(8\d{2}|[1-9]\d{3})\b/.test(lower)) score += 0.10;

  // Positive: official brand domain
  if (sourceDomain) {
    const domainLower = sourceDomain.toLowerCase();
    const isOfficial = /makita\.|dewalt\.|bosch-professional\.|milwaukeetool\.|hilti\.|metabo\.|festool\.|hikoki\./.test(domainLower);
    if (isOfficial) score += 0.15;
    // W-IA-2: Domain-specific CDN bonus — these retailers consistently deliver catalog-quality shots
    const isHighQualityCDN = HIGH_QUALITY_CDN_DOMAINS.some(d => domainLower.includes(d));
    if (isHighQualityCDN) score += 0.10;
  }

  // Negative: thumbnail/small indicators
  if (/\b(thumb|thumbnail|_s\.|_small|_xs|_tiny|preview|mini)\b/.test(lower)) score -= 0.20;
  if (/[?&](w|width)=([1-2]\d{2})\b/.test(lower)) score -= 0.15; // Width 100-299px

  // Negative: watermark/overlay indicators
  if (/watermark|overlay|stamp/.test(lower)) score -= 0.15;

  // W-IA-2: Negative — lifestyle/action/hero shots (not pure product-on-white images)
  if (/\/(hero|lifestyle|action|in[_-]?use|banner[_-]?|promo|ambient)\//i.test(imageUrl)) score -= 0.15;

  // Negative: heavily compressed
  if (/[?&](q|quality)=([1-4]\d)\b/.test(lower)) score -= 0.10;
