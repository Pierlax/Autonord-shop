...
/** Maximum raw XML size accepted (50 MB) — prevents OOM before parsing */
const MAX_XML_SIZE_BYTES = 50 * 1024 * 1024;

/**
 * Parse Danea EasyFatt XML content.
 *
 * @throws {Error} if the raw string exceeds MAX_XML_SIZE_BYTES (pre-parse OOM guard).
 * @throws {Error} if the document exceeds MAX_PRODUCTS items (post-parse OOM guard).
 */
export function parseDaneaXML(xmlContent: string): DaneaXMLParseResult {
  // Guard BEFORE parsing — a huge payload would exhaust memory inside XMLParser
  if (xmlContent.length > MAX_XML_SIZE_BYTES) {
    throw new Error(
      `[DaneaXML] Payload too large: ${(xmlContent.length / 1024 / 1024).toFixed(1)} MB ` +
      `exceeds the maximum of ${MAX_XML_SIZE_BYTES / 1024 / 1024} MB. Split the export into smaller batches.`
    );
  }
