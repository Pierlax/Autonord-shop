...
export function checkKrugCompliance(text: string): {
  ...
  // D18 fix: Cognitive clarity checks — "Don't Make Me Think" goes beyond length.
  // A 150-word text can be incomprehensible if it lacks structure.

  // D18a: First-sentence clarity — the first sentence should state the problem or product
  // function, not be a generic marketing opener.
  if (sentences.length > 0) {
    const firstSentence = sentences[0].trim().toLowerCase();
    const genericOpeners = [
      'con questo', 'grazie a', 'finalmente', 'ecco', 'scopri',
      'quando si tratta di', 'nel mondo di', 'se stai cercando',
    ];
    if (genericOpeners.some(opener => firstSentence.startsWith(opener))) {
      issues.push('Prima frase generica — dovrebbe dichiarare subito il problema che risolve');
      score -= 10;
    }
  }

  // D18b: Information density — check for "empty" sentences (many words, no numbers or specifics)
  const emptySentences = sentences.filter(s => {
    const words = s.trim().split(/\s+/);
    if (words.length < 8) return false; // Short sentences are fine
    const hasNumber = /\d/.test(s);
    const hasSpecificNoun = /\b(mm|cm|kg|nm|rpm|volt|watt|bar|litri|ore|anni|ampere|ah|kwh|kw|hp)\b/i.test(s);
    return !hasNumber && !hasSpecificNoun;
  });
  if (emptySentences.length > 2) {
    issues.push(`${emptySentences.length} frasi lunghe senza dati concreti — sostituire con fatti`);
    score -= emptySentences.length * 3;
  }

  // D18c: Repetition check — same key phrase appearing 3+ times signals lazy generation
  const phrases = text.toLowerCase().match(/\b\w{4,}\s+\w{4,}\b/g) || [];
  const phraseCounts = new Map<string, number>();
  for (const p of phrases) {
    phraseCounts.set(p, (phraseCounts.get(p) || 0) + 1);
  }
  const repetitions = Array.from(phraseCounts.entries()).filter(([, c]) => c >= 3);
  if (repetitions.length > 0) {
    issues.push(`Ripetizioni eccessive: ${repetitions.map(([p, c]) => `"${p}" ×${c}`).join(', ')}`);
...
export function checkJTBDCompliance(text: string): {
  ...
  // D19 fix: Verify that specs are actually connected to work benefits.
  // A text mentioning "idraulico" without linking a spec to their workflow
  // passes the old keyword check but fails the JTBD spirit.

  // D19a: Count spec-to-benefit connections (number + benefit language nearby)
  // Pattern: a number/unit followed within 50 chars by a benefit phrase
  const specBenefitPattern = /\d+\s*(?:nm|kg|rpm|v|ah|watt|w|bar|litri|ore|mm|cm|kw|hp)\b[^.]{0,50}(?:per|consente|permette|significa|ideale|sufficiente|gestibile|comodo|pratico|efficiente|adatto|pensato)/gi;
  const specBenefitMatches = text.match(specBenefitPattern) || [];
  if (specBenefitMatches.length === 0) {
    // Check if there are specs at all but no benefit connections
    const hasSpecs = /\d+\s*(?:nm|kg|rpm|v|ah|watt|w|bar|litri|ore|mm|cm|kw|hp)\b/i.test(text);
    if (hasSpecs) {
      issues.push('Specifiche presenti ma non collegate a benefici lavorativi concreti (JTBD)');
      score -= 10;
    }
  }

  // D19b: Check for trade-specific benefit language (not just mentioning a trade)
  // Good: "per l'idraulico che lavora in spazi stretti: solo 1.5 kg"
  // Bad:  "ideale per idraulici" (keyword-only, no concrete benefit)
  const tradesMentioned = Object.keys(JTBD_TRADE_CONTEXTS).filter(trade => lowerText.includes(trade));
  if (tradesMentioned.length > 0) {
    // Check that at least one trade mention is near a spec or benefit
    let tradeWithBenefit = false;
    for (const trade of tradesMentioned) {
      const tradeIdx = lowerText.indexOf(trade);
      const nearby = lowerText.slice(Math.max(0, tradeIdx - 80), tradeIdx + trade.length + 80);
      if (/\d/.test(nearby) || /perché|grazie|consente|permette|spazi|cantiere|lavoro/.test(nearby)) {
        tradeWithBenefit = true;
        break;
      }
    }
    if (!tradeWithBenefit) {
      issues.push('Mestieri menzionati ma senza collegamento a benefici specifici');
      score -= 8;
    }
  }
