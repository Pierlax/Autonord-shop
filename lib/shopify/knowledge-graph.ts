...
export class PowerToolKnowledgeGraph {
  ...
  enrichProductContext(
    ...
    // W5 fix: Dynamic feature inference from product name when category map misses.
    // Regex patterns detect feature keywords in the product name itself,
    // so new/unmapped categories get relevant features instead of generic fallbacks.
    if (suggestedFeatures.length === 0) {
      const dynamicFeatures = this.inferFeaturesFromProductName(productName);
      suggestedFeatures.push(...dynamicFeatures);
    }

    // Ultimate fallback: only if dynamic inference also found nothing
    if (suggestedFeatures.length === 0) {
  ...
  /**
   * W5 fix: Infer features dynamically from the product name.
   *
   * When the static CATEGORY_FEATURE_MAP has no entry for the inferred category,
   * this method scans the product name for keyword patterns that indicate
   * specific features. This handles new product types (e.g. plasma cutters,
   * laser levels) without requiring a static map update.
   */
  private inferFeaturesFromProductName(productName: string): KGNode[] {
    const text = productName.toLowerCase();
    const FEATURE_PATTERNS: Array<{ pattern: RegExp; featureId: string }> = [
      { pattern: /brushless|motore brushless|senza spazzole|bl\b/,          featureId: 'feature_brushless' },
      { pattern: /velocità variabile|variable speed|regolazione velocità/,   featureId: 'feature_variable_speed' },
      { pattern: /\bled\b|illuminazione|luce lavoro/,                        featureId: 'feature_led_light' },
      { pattern: /bluetooth|app|wireless|one[- ]?key/,                       featureId: 'feature_bluetooth' },
      { pattern: /aspirazione|aspiratore|dust extract/,                      featureId: 'feature_dust_extraction' },
      { pattern: /kickback|anti[- ]?contraccolpo/,                           featureId: 'feature_kickback_protection' },
      { pattern: /diesel|gasolio/,                                           featureId: 'feature_diesel_engine' },
      { pattern: /idraulic|hydraulic/,                                       featureId: 'feature_hydraulic_system' },
      { pattern: /avviamento elettrico|electric start|e-start/,              featureId: 'feature_electric_start' },
      { pattern: /avr|regolazione tensione|voltage regul/,                   featureId: 'feature_avr_regulation' },
      { pattern: /attacco rapido|quick coupler|sgancio rapido/,              featureId: 'feature_quick_coupler' },
      { pattern: /disco diamant|diamond blade|diamantato/,                   featureId: 'feature_diamond_blade' },
    ];

    const features: KGNode[] = [];
    const seen = new Set<string>();
    for (const { pattern, featureId } of FEATURE_PATTERNS) {
      if (pattern.test(text) && !seen.has(featureId)) {
        const node = this.graph.nodes.get(featureId);
        if (node) {
          features.push(node);
          seen.add(featureId);
        }
