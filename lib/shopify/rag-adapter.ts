...
// SourceType is re-exported from rag-evidence-extractor (single source of truth)
export type { SourceType };

/**
 * A single evidence source with provenance metadata.
 * Replaces the flat `sourceData` string so TwoPhaseQA can weight sources logically.
 */
export interface StructuredSource {
  /** URL or domain identifier of the source */
  source: string;
  /** Inferred semantic type */
  type: SourceType;
  /** Trust score 0–1 (1 = official spec sheet, 0.4 = forum post) */
  trust: number;
  /** Extracted text content from this source */
  text: string;
  /** True when this source's values are known to conflict with other sources */
  hasConflict?: boolean;
}
...
export interface TwoPhaseQAInput {
  ...
  category: string;
  /**
   * @deprecated Prefer `structuredSources`. Still used as a fallback when
   * structuredSources is unavailable (e.g. empty RAG result).
   */
  sourceData?: string;
  /**
   * Structured evidence from the RAG pipeline, ordered by trust score.
   * When present, TwoPhaseQA uses this instead of the flat `sourceData` string,
   * enabling source-weighted fact extraction and conflict-aware reasoning.
   */
...
// SOURCE TYPE INFERENCE HELPERS are imported from rag-evidence-extractor.ts
// (SourceType, inferSourceType, getTrustScore — single source of truth)
...
export function adaptRagToQa(
  ...
  // Structured sources array — preserves provenance for TwoPhaseQA prompt rendering
  ...
  if (data.evidence && Array.isArray(data.evidence)) {
    for (const item of data.evidence) {
      ...
      if (typeof item === 'string') {
        sections.push(item);
        const srcType = inferSourceType(sourceLabel);
        structuredSources.push({ source: sourceLabel, type: srcType, trust: getTrustScore(srcType), text: item });
      } else if (item.content || item.text || item.snippet) {
        const text: string = item.content || item.text || item.snippet || '';
        const confidence = item.confidence ? ` [confidence: ${item.confidence}]` : '';
        sections.push(`[Source: ${sourceLabel}${confidence}]\n${text}`);
        const srcType = inferSourceType(sourceLabel, item.sourceType);
        structuredSources.push({ source: sourceLabel, type: srcType, trust: getTrustScore(srcType), text });
        ...
        const text = `${item.field}: ${item.value}`;
        sections.push(`[${item.evidenceType || 'evidence'}${verified} | ${sourceList}]\n${text}`);
        const srcType = inferSourceType(sourceList, item.evidenceType);
        // isVerified evidence from spec sheets gets higher trust
        const trust = item.isVerified ? Math.max(getTrustScore(srcType), 0.85) : getTrustScore(srcType);
        structuredSources.push({ source: sourceList, type: srcType, trust, text });
        ...
        if (relevantFields) {
          ...
          const srcType = inferSourceType(sourceLabel);
          structuredSources.push({ source: sourceLabel, type: srcType, trust: getTrustScore(srcType), text: relevantFields });
        }
      }
    }
  }
  ...
  if (data.v2CorpusContext && typeof data.v2CorpusContext === 'string' && data.v2CorpusContext.length > 20) {
    ...
    // V2 corpus is pre-typed by CorpusBuilder — assign spec_sheet trust as a reasonable default
    structuredSources.unshift({
      source: 'v2_corpus',
      type: 'spec_sheet',
      trust: 0.85,
      text: data.v2CorpusContext,
    });
  }
  if (data.v2EvidenceGraphContext && typeof data.v2EvidenceGraphContext === 'string' && data.v2EvidenceGraphContext.length > 20) {
    ...
    structuredSources.push({
      source: 'v2_evidence_graph',
      type: 'benchmark',
      trust: 0.80,
      text: data.v2EvidenceGraphContext,
    });
  }
  ...
  if (data.benchmarkContext && typeof data.benchmarkContext === 'string') {
    ...
    if (!contributingSources.includes('benchmark')) contributingSources.push('benchmark');
    structuredSources.push({
      source: 'benchmark_context',
      type: 'benchmark',
      trust: getTrustScore('benchmark'),
      text: data.benchmarkContext,
    });
  }
  ...
  if (data.brandProfile) {
    ...
    structuredSources.push({
      ...
      type: 'brand_site',
      trust: getTrustScore('brand_site'),
      text: brandInfo,
    });
  }
  ...
  // Mark conflicts as a dedicated entry so TwoPhaseQA knows which facts are contested.
  if (data.conflicts && Array.isArray(data.conflicts) && data.conflicts.length > 0) {
    ...
    structuredSources.push({
      source: 'evidence_graph_conflicts',
      type: 'unknown',
      trust: 0.50,
      text: `CONFLITTI RILEVATI TRA FONTI:\n${conflictInfo}`,
      hasConflict: true,
    });
  }
  ...
  // Sort structured sources by trust descending so TwoPhaseQA sees the best
  // evidence first when it renders the prompt section.
  structuredSources.sort((a, b) => b.trust - a.trust);
  ...
  return {
    qaInput: {
      ...
      structuredSources: structuredSources.length > 0 ? structuredSources : undefined,
    },
    metadata: {
      ragSucceeded: true,
      evidenceCount,
