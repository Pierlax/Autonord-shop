...
class RAGCacheManager {
   ...
   *
   * D2 fix: Verifies the stored query+domains match the requested ones.
   * djb2 is a 32-bit hash — with ~65k unique queries the birthday paradox
   * gives ~50% collision probability. By checking the stored original query
   * we detect collisions and treat them as cache misses.
   */
  async get(query: string, domains?: string[]): Promise<CacheEntry | null> {
    ...
    if (entry) {
      // D2 fix: collision detection — verify stored query matches requested query
      const normalizedQuery = query.toLowerCase().trim();
      const normalizedDomains = domains ? [...domains].sort().join(',') : '';
      const storedQuery = (entry.query || '').toLowerCase().trim();
      const storedDomains = (entry.domains || []).sort().join(',');

      if (storedQuery !== normalizedQuery || storedDomains !== normalizedDomains) {
        this.stats.misses++;
        log.warn(
          `[RAGCache] COLLISION detected for key "${key}": ` +
          `requested="${normalizedQuery.substring(0, 40)}" vs stored="${storedQuery.substring(0, 40)}"`
        );
        return null;
      }
