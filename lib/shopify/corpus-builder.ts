...
/**
 * W6 fix: realistic token estimation.
 *
 * The naive `length / 4` underestimates actual prompt tokens because:
 * - Markdown overhead (headers, source URLs, separators) is ~80 tokens per item
 * - Italian/accented text averages ~3.2 chars per token, not 4
 * - JSON key overhead from downstream rendering adds ~10-40 tokens
 *
 * New formula accounts for the rendering overhead so the budget is
 * respected downstream and content is neither truncated nor under-utilized.
 */
function estimateTokens(text: string): number {
  // Base: Italian text averages ~3.2 chars/token (vs 4 for English)
  const baseTokens = Math.ceil(text.length / 3.2);
  // Markdown rendering overhead per corpus item (header + URL + separator)
  const RENDERING_OVERHEAD = 80;
  return baseTokens + RENDERING_OVERHEAD;
}
...
export function buildCorpus(
  ...
  for (const res of navigatedResources) {
    ...
    // W1 fix: comprehensive mock filter — check text markers AND mock URL patterns
    if (
      content.includes('[MOCK') ||
      res.title.includes('[MOCK]') ||
      content.includes('[MOCK DATA]') ||
      /mock-result-\d+/.test(res.url)
    ) {
      log.warn(`[CorpusBuilder] Skipping mock item: ${res.url}`);
      continue;
    }
  ...
  for (const src of discoveredSources) {
    ...
    // W1 fix: comprehensive mock filter — provider flag + text markers + URL patterns
    if (
      src.provider === 'mock' ||
      content.includes('[MOCK') ||
      content.includes('[MOCK DATA]') ||
      src.title.includes('[MOCK]') ||
      /mock-result-\d+/.test(src.url)
    ) {
      log.warn(`[CorpusBuilder] Skipping mock source (provider=${src.provider}): ${src.url}`);
      continue;
    }
    const tokens = estimateTokens(content);
    if (tokens === 0) continue;
...
export function corpusToContext(corpus: CorpusCollection, maxTokens = 4000): string {
  ...
  // D6 fix: Per-item separator overhead in tokens.
  // Each item adds: "[TYPE: title]\nFonte: url\n" header (~30-60 chars → ~10-15 tokens)
  // plus "\n\n---\n\n" separator between items (~3 tokens).
  // Previously only item.tokenEstimate (raw content) was counted, causing
  // the rendered context to exceed maxTokens by 20-30%.
  const SEPARATOR_OVERHEAD_TOKENS = 3; // "\n\n---\n\n"
  ...
  for (const type of typeOrder) {
    for (const item of corpus.byType[type] ?? []) {
      ...
      // D6 fix: Account for header + URL line + separator tokens
      const headerLine = `${header}\nFonte: ${item.url}\n`;
      const headerTokens = Math.ceil(headerLine.length / 4);
      const totalItemTokens = item.tokenEstimate + headerTokens +
        (sections.length > 0 ? SEPARATOR_OVERHEAD_TOKENS : 0);

      if (usedTokens + totalItemTokens > maxTokens) continue;

      sections.push(`${headerLine}${item.content}`);
 ...
 *
 * W1 fix: provider is set to 'bing' (not 'mock') because corpus items have already
 * been validated and mock-filtered by buildCorpus(). Marking them as 'mock' caused
 * the downstream Mock Gate to strip legitimate pre-processed corpus data.
 */
