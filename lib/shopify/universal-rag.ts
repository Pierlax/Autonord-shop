...
export interface UniversalRAGResult {
   ...
   */
  visualClues?: string[];
  /** D8 fix: Structured metrics for observability, alerting, and trend analysis */
  metrics?: PipelineMetrics;
  debugLog: string[];
}

/**
 * W8 fix: Per-pipeline AI call budget.
 *
 * Tracks how many generateTextSafe() calls this pipeline run has consumed
 * vs. how many it's allowed. Optional calls (visual clues, LLM evaluation)
 * check the remaining budget before firing, preventing rate-limit starvation
 * when multiple products are enriched concurrently.
 */
interface AICallBudget {
  /** Max AI calls allowed for this pipeline run (default: 4). */
  maxCalls: number;
  /** Number of AI calls consumed so far. */
  usedCalls: number;
}

/**
 * D8 fix: Structured observability metrics for trend analysis and alerting.
 *
 * Tracks latency per layer, cache performance, quality scores, and token usage
 * in a machine-parseable format. The previous debugLog (string[]) is kept for
 * human-readable logs; metrics are the structured counterpart.
 */
export interface PipelineMetrics {
  /** Latency in ms per pipeline layer */
  latencyByLayer: {
    ...
    evidenceGraph?: number;
    evaluatorOptimizer?: number;
    groundedVision?: number;
  };
  /** Cache hit/miss/collision counts for this run */
  cache: {
    hits: number;
    misses: number;
    collisions: number;
  ...
  /** Number of semantic dedup removals (D5) */
  dedupRemovals: number;
  /** Bing health check status (D1) */
  bingHealthy?: boolean;
}
...
interface PipelineState {
  ...
  tokensUsed: number;
  costSavings: number;
  /** W8: per-pipeline AI call budget */
  aiBudget: AICallBudget;
  /** D8: structured metrics */
  metrics: PipelineMetrics;
}
...
export class UniversalRAGPipeline {
  ...
  async enrichProduct(
    ...
    const state: PipelineState = {
      ...
      tokensUsed: 0,
      costSavings: 0,
      aiBudget: { maxCalls: 4, usedCalls: 0 },
      metrics: {
        latencyByLayer: {},
        cache: { hits: 0, misses: 0, collisions: 0 },
        quality: {},
        tokens: { estimated: 0, budgetMax: this.config.maxTokenBudget, renderingOverhead: 0 },
        aiCalls: { used: 0, max: 4 },
        dedupRemovals: 0,
      },
    };
    ...
    if (this.config.enableBenchmarkContext) {
      ...
      state.metrics.latencyByLayer.benchmarkLoad = Date.now() - _t0;
    ...
    if (this.config.enableSourceDiscovery) {
      ...
      state.metrics.latencyByLayer.v2Discovery = Date.now() - _tDisc;
    ...
    try {
      ...
      if (this.config.enableNoRetrievalDetection) {
        state.aiBudget.usedCalls++; // W8: track AI call
      ...
      state.metrics.latencyByLayer.retrieval = Date.now() - _tRetrieval;
      ...
      // D3 note: KG hydration is handled by the route handler (kgStore.hydrateKG)
      // before the pipeline runs. No auto-hydration here to avoid pulling
      // kg-store → episodic-bridge → fs into the client webpack bundle.
      ...
      state.metrics.latencyByLayer.knowledgeGraph = Date.now() - _tKG;
      ...
      // W8: Guarded by AI call budget — skip if budget exhausted.
      let visualClues: string[] = [];
      if (
        (enrichedData.v2CorpusContext?.length ?? 0) > 100 &&
        state.aiBudget.usedCalls < state.aiBudget.maxCalls
      ) {
        state.aiBudget.usedCalls++;
      ...
      } else if ((enrichedData.v2CorpusContext?.length ?? 0) > 100) {
        this.log(state, `W8: Skipping visual clues extraction — AI budget exhausted (${state.aiBudget.usedCalls}/${state.aiBudget.maxCalls})`);
      }

      // D8 fix: Finalize structured metrics before creating result
      state.metrics.tokens.estimated = state.tokensUsed;
      state.metrics.aiCalls.used = state.aiBudget.usedCalls;
      state.metrics.aiCalls.max = state.aiBudget.maxCalls;
      if (v2Corpus) {
        state.metrics.quality.corpusCoverage = v2Corpus.coverageScore;
      }
  ...
  private async executeSourceAwareRetrieval(
    ...
    // D5 fix: Deduplicate results by URL AND by content similarity.
    // Three pages from different retailers with the same copy-pasted spec sheet
    // were inflating coverage score and fake-triangulating. Now we detect near-duplicate
    // content via normalized text fingerprinting (Jaccard on word n-grams).
    const dedupedResults = this.deduplicateResults(allResults);
    if (dedupedResults.length < allResults.length) {
      log.info(`[UniversalRAG] D5 dedup: ${allResults.length} → ${dedupedResults.length} results for source ${source}`);
    } else {
    ...
    const mappedResults = dedupedResults.map(result => ({
      source,
      sourceType: source,
      intent,
  ...
  private createResult(
    ...
    return {
      ...
      // D8 fix: Attach structured metrics for observability
  ...
  /**
   * D5 fix: Deduplicate search results by URL and by content similarity.
   *
   * Step 1: URL dedup (exact match on normalized URL).
   * Step 2: Content dedup — compute word 3-gram sets and measure Jaccard
   * similarity. Results with > 70% overlap are considered duplicates;
   * the one from the higher-trust domain is kept.
   */
  private deduplicateResults(results: SearchResult[]): SearchResult[] {
    ...
    for (const r of results) {
      const normUrl = r.link.replace(/\/+$/, '').toLowerCase();
      if (urlSeen.has(normUrl)) continue;
      urlSeen.add(normUrl);
      urlDeduped.push(r);
    }

    // Step 2: Content dedup via Jaccard on word 3-grams
    const getNgrams = (text: string): Set<string> => {
      const words = text.toLowerCase().replace(/[^a-z0-9àèéìòùü\s]/g, '').split(/\s+/).filter(w => w.length > 2);
    ...
    const fingerprints = urlDeduped.map(r => getNgrams(r.snippet || ''));
    const keep = new Array(urlDeduped.length).fill(true);

    for (let i = 0; i < urlDeduped.length; i++) {
      if (!keep[i]) continue;
      for (let j = i + 1; j < urlDeduped.length; j++) {
        if (!keep[j]) continue;
        // Only compare if both have meaningful snippets
        if (fingerprints[i].size < 3 || fingerprints[j].size < 3) continue;
        if (jaccard(fingerprints[i], fingerprints[j]) > 0.70) {
          // Keep the one from a whitelisted domain (higher trust)
          const iWhitelisted = isWhitelistedDomain(urlDeduped[i].link);
          const jWhitelisted = isWhitelistedDomain(urlDeduped[j].link);
          if (jWhitelisted && !iWhitelisted) {
            keep[i] = false;
            break; // i is gone, no need to compare further
          } else {
            keep[j] = false;
          }
        }
    ...
    return urlDeduped.filter((_, idx) => keep[idx]);
  }

  /**
   * Estimate tokens from content.
   *
   * D6 fix: Uses length/3.5 (closer to real BPE tokenization for mixed IT/EN text)
   * plus a fixed overhead per item for markdown rendering (headers, URLs, separators)
   * that corpusToContext() adds. Previously used length/4 with no overhead,
   * causing the actual token count to overshoot the budget by 20-30%.
   */
  private estimateTokens(content: any): number {
    const str = JSON.stringify(content);
    const RENDERING_OVERHEAD_PER_ITEM = 20; // header + URL + separator tokens
    const itemCount = Array.isArray(content) ? content.length : 1;
    return Math.ceil(str.length / 3.5) + (itemCount * RENDERING_OVERHEAD_PER_ITEM);
  }
  ...
  private async runV2Discovery(
    ...
    if (this.config.enableCorpusBuilder) {
      ...
      // W7 fix: If corpus is too small and cross-domain hop was not enabled,
      // retry navigation with cross-hop enabled to broaden coverage.
      if (
        corpus.totalItems < 5 &&
        this.config.enableDomainNavigation &&
        !navigationResult.debugLog.some(l => l.includes('Cross-hop candidates'))
      ) {
        this.log(state, `W7: Corpus too small (${corpus.totalItems} items), retrying with cross-domain hop`);
        const crossHopBudget: Partial<NavigationBudget> = {
          maxUrlsPerDomain: this.config.navigationBudgetPerDomain,
          maxTotalUrls: 10,
          maxDepth: 2,
          enableCrossDomainHop: true,
        };
        const crossHopResult = await navigateDomains(discoveryResult.sources, crossHopBudget);
        // Merge new resources and rebuild corpus
        const mergedResources = [
          ...navigationResult.resources,
          ...crossHopResult.resources.filter(
            r => !navigationResult.resources.some(existing => existing.url === r.url)
          ),
        ];
  ...
  private async runEvaluatorOptimizer(
    ...
    // W8: pass AI budget so LLM eval respects the per-pipeline limit
    const startEval = await evaluateCorpus(corpus, evidenceGraph, productTitle, vendor, state.aiBudget);
    if (startEval.reasoning.includes('LLM') || startEval.reasoning.includes('score=')) {
      state.aiBudget.usedCalls++; // LLM path was taken
    }
    this.log(
      state,
    ...
    for (let pass = 1; pass <= this.config.maxRetrievalPasses; pass++) {
      ...
      const newEval = await evaluateCorpus(updatedCorpus, evidenceGraph, productTitle, vendor, state.aiBudget);
      if (newEval.reasoning.includes('LLM') || newEval.reasoning.includes('score=')) {
        state.aiBudget.usedCalls++;
      }
 ...
 *
 * D7 fix: Added explicit page cap (MAX_IMAGE_PAGES) and overall wall-clock timeout
 * (MAX_IMAGE_EXTRACTION_MS). With 20+ ragPageUrls this step was adding 10-30s of
 * latency without contributing to text quality. Now hard-capped at 6 pages / 8s.
 */
const MAX_IMAGE_PAGES = 6;
const MAX_IMAGE_EXTRACTION_MS = 8_000;

async function extractProductImageCandidates(pageUrls: string[]): Promise<string[]> {
  // D7 fix: Cap the number of pages to prevent excessive latency
  const cappedUrls = pageUrls.slice(0, MAX_IMAGE_PAGES);

  const seen = new Set<string>();

  // D7 fix: Race all fetches against a wall-clock timeout
  const fetchAll = Promise.allSettled(cappedUrls.map(u => fetchImageCandidatesFromPage(u)));
  const timeoutPromise = new Promise<PromiseSettledResult<string[]>[]>(resolve =>
    setTimeout(() => resolve([]), MAX_IMAGE_EXTRACTION_MS)
  );
