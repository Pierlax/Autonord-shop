...
export async function navigateDomains(
  ...
  if (effectiveBudget.maxDepth >= 1 && resources.length < effectiveBudget.maxTotalUrls) {
    ...
    // W3 fix: filter depth-0 resources before depth-1 expansion.
    // Skip resources that are already PDFs/specs (low ROI from further expansion)
    // and low-confidence resources when budget is >70% exhausted.
    const exhaustionRate = resources.length / effectiveBudget.maxTotalUrls;
    const depth1Candidates = resources.filter(r => {
      // Already a PDF or spec — depth-1 won't find much more
      if (r.resourceType === 'pdf' || r.resourceType === 'spec') return false;
      // Budget tight + low confidence → skip
      if (exhaustionRate > 0.7 && r.confidence < 0.75) return false;
      return true;
    }).slice(0, 4); // still cap at 4

    const depth1 = await expandByPatternQueries(
      depth1Candidates.length > 0 ? depth1Candidates : resources.slice(0, 2),
      effectiveBudget,
      domainCounts,
      seenUrls,
      debugLog
