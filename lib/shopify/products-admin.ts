...
function transformProduct(adminProduct: any): Product {
  ...
  return {
    ...
    metafields: buildMetafields(adminProduct.metafields?.edges),
  };
}

function buildMetafields(
  edges: Array<{ node: { key: string; namespace: string; value: string; type: string } }> | undefined
): Product['metafields'] {
  if (!edges || edges.length === 0) return undefined;

  const map: Record<string, { key: string; namespace: string; value: string; type: string }> = {};
  for (const edge of edges) {
    map[edge.node.key] = edge.node;
  }
